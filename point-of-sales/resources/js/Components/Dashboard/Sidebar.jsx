...
export default function Sidebar({ sidebarOpen }) {
    ...
    return (
        <div className={`${sidebarOpen ? 'w-[260px]' : 'w-[100px]'} hidden md:block min-h-screen overflow-y-auto border-r transition-all duration-300 bg-amber-50 dark:bg-gray-950 dark:border-gray-900`}>
            {sidebarOpen ? (
                <>
                    <div className="flex justify-center items-center px-6 py-2 h-16">
                        <div className="text-2xl font-bold text-center leading-loose tracking-wider text-cyan-600 dark:text-gray-200">
                            Point Of Sales
                        </div>
                    </div>
                    <div className="w-full p-3 flex items-center gap-4 border-b border-t dark:bg-gray-950/50 dark:border-gray-900">
                        ...
                        <div className="flex flex-col gap-0.5">
                            ...
                            <div className="text-xs text-gray-950 dark:text-gray-400">
                                {"shekul.bd201@gmail.com"}
                            </div>
                        </div>
                    </div>
                    <div className="w-full flex flex-col overflow-y-auto">
                        {menuNavigation.map((item, index) => (
                            item.details.some(detail => detail.permissions === true) && (
                                <div key={index}>
                                    <div className="text-gray-950 text-xs py-3 px-4 font-bold uppercase">
                                        {item.title}
                                    </div>
